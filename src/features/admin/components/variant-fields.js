...
export function mountVariantFields(root, initial, assets, track, report, signal) {
  ...
  root.addEventListener('change', async event => {
    if (event.target.dataset.variantImage === undefined || !event.target.files[0]) return;
    const index = Number(event.target.dataset.variantImage);
    const rowId = variants[index];
    try {
      const asset = await assets.uploadProductImage(event.target.files[0]);
      if (signal.aborted) { assets.release(asset.url); return; }
      track(asset.url); const current = variants.indexOf(rowId);
      if (current < 0) return;
      variants = read(); variants[current].image = asset.url; render();
