...
export async function openCheckout(){
  ...
  dialog.addEventListener('submit',async event=>{
    ...
    try{
      if(step===3){order=await checkoutService.confirm({review,shippingAddress:{...shipping,recipientName:`${personal.firstName} ${personal.lastName}`},userId:user.id,requestId});step=4;}
