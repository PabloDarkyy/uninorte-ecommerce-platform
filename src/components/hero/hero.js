import { mountHeroRotation } from './hero-rotation.js';
...
export function hero(product) {
  return `<section class="hero-transition" id="home" aria-labelledby="hero-title" tabindex="-1"><div class="hero-sticky"><div class="hero"><div class="hero-copy"><p class="eyebrow"><span></span> Productos paraguayos</p>
    ...
    <div class="hero-exhibit">${productStage(product, { context: 'hero' })}<div class="exhibit-bottomline"><span>Una colección con raíces.</span><span class="exhibit-seal">PY</span></div></div></div><div class="hero-canvas-overlay" aria-label="Producto 3D del hero"></div></div></section>`;
}

export function mountHero(root, product, products = [product]) {
  ...
  const disposeRotation=mountHeroRotation(root,viewer,products,product);
  return () => { disposeRotation();disposeScroll(); viewer.dispose(); };
}
