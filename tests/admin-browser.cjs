...
(async () => {
 ...
 try {
  ...
  await form.locator('[data-variant-model]').setInputFiles('src/assets/models/menta.glb');await form.getByText('Modelo asociado',{exact:true}).waitFor();
  ...
  assert.ok(await page.evaluate(async id=>{const{productService}=await import('/src/services/product.service.js');return(await productService.getById(id)).variants[0].model3dUrl.startsWith('blob:');},id));
